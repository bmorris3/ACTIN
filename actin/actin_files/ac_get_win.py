...
def sel_order(wave_2d, ln_ctr, ln_win, file_type):
    ...
    if file_type == "S2D": ord_type = "espresso"
    else: ord_type = "harps" #####
    ...
    if ord_type == "harps":
        ...
        if ln_ctr != 3933.664: orders = [orders[-1]]

    if ord_type == "espresso":
        # first two for CaIIK
        if ln_ctr == 3933.664: orders = orders[:-2]
        # last two for CaIIH
        if ln_ctr == 3968.47: orders = orders[2:]
...
def line_plot(data, ord, ln_id, ln_ctr, ln_win, bandfunc, bandtype, out_dir):
    ...
    if isinstance(ord, int):
        ...
        print("***ERROR: ord must be either 'None' or instance of 'int'.")
